...
//Clasa CartCrudGUI (contine lista de carti din cos, butonul de generare si cel de golire)
class CartCrudGui : public Observer
{public:
    Biblioteca& serv;
    vector<Observer*>& observers;

    QWidget* cartWindow = new QWidget();
    QPushButton* btnGenerateCart = new QPushButton("&Genereaza cos");
 ...
 public:
    CartCrudGui(Biblioteca& service, vector<Observer*>& observers) : serv{ service }, observers{ observers } {
    ...
    void initGUI() {
        ...
        cartWindow->setLayout(lyMain);
        //setLayout(lyMain);
        lyMain->addWidget(lst);
        lyMain->addWidget(total);
        lyMain->addWidget(btnEmptyCart);
        QHBoxLayout* lyGenerate = new QHBoxLayout();
        lyGenerate->addWidget(btnGenerateCart);
...
//Clasa CartReadOnly (contine atatea linii/figuri geometrice cate carti se afla in cos)
class CartReadOnlyGui : public Observer, QWidget
{
public:
    Biblioteca& serv;
    vector<Observer*>& observers;
...
public:
    CartReadOnlyGui(Biblioteca& service, vector<Observer*>& observers) : serv{ service }, observers{ observers } {
        this->setLayout(cartLayout);
        this->setFixedHeight(615);
        this->setFixedWidth(615);
        update();
        this->show();
    }
    ...
    void paintEvent(QPaintEvent* ev) override {
        ev;
        QPainter p{ this };
        auto n = serv.getAllBooksFromCart().size();
        for (int i = 0; i < n; i++) {
            std::random_device rdm;
            std::mt19937 eng{ rdm() };
            std::uniform_int_distribution<int> random(0, 1);
            int r = random(eng);
            int x = rand() % 415, y = rand() % 415;
            if (r == 0) p.drawRect(rand() % 415, rand() % 415, rand() % 415 - x, rand() % 415 - y);
            if (r == 1) p.drawLine(rand() % 415, rand() % 415, rand() % 415 - x, rand() % 415 - y);
...
class BibliotecaGUIObserver : public QWidget { 
...
private:
    ...
    QPushButton* btnEmptyCart = new QPushButton("&Goleste cosul");
    QPushButton* btnGenerateCart = new QPushButton("&Genereaza cos");
    QPushButton* btnAddCart = new QPushButton("&Adauga in cos");
    ...
    QSlider* sliderYear = new QSlider(Qt::Horizontal);
    QSpinBox* spinGenerate = new QSpinBox();

public:
    BibliotecaGUIObserver(Biblioteca& service) : serv{ service } {
    ...
    void initGUI() {
        ...
        sliderYear->setMaximum(2021);
        sliderYear->setMinimum(1700);
        sliderYear->setValue(1);
        rightLy->addWidget(sliderYear);
       ...
       // layout butoane cart
        ...
        lyCart->addWidget(btnCosCrudGui);
        lyCart->addWidget(btnCosReadOnlyGui);
        lyCart->addWidget(btnAddCart);
        lyCart->addWidget(btnEmptyCart);

        QHBoxLayout* lyGenerate = new QHBoxLayout{};
        lyGenerate->addWidget(btnGenerateCart);
        lyGenerate->addWidget(txtGenerate);
        //spinGenerate->setStyleSheet("QSpinBox {background-color: #596866; color: black;}");
        spinGenerate->setMinimum(1);
        spinGenerate->setMaximum(7);
        spinGenerate->setSingleStep(1);
        lyGenerate->addWidget(spinGenerate);
        lyCart->addLayout(lyGenerate);
        
        QHBoxLayout* lyHTML = new QHBoxLayout{};
        lyHTML->addWidget(btnHTML);
        lyHTML->addWidget(txtHTML);
        lyCart->addLayout(lyHTML);

        QHBoxLayout* lyCSV = new QHBoxLayout{};
        lyCSV->addWidget(btnCSV);
        lyCSV->addWidget(txtCSV);
        lyCart->addLayout(lyCSV);

      
        lyMain->addLayout(lyCart);
    }

    void initConnect() {
        QObject::connect(sliderYear, &QSlider::valueChanged, this, [=]() {
            txtYear->setText(QString::number(sliderYear->value()));
            });

        QObject::connect(spinGenerate, &QSpinBox::valueChanged, this, [=]() {
            txtGenerate->setText(QString::number(spinGenerate->value()));
            });
        ...
        QObject::connect(tblView->selectionModel(), &QItemSelectionModel::selectionChanged, [this]() {
            ...
            txtAuthor->setText(cellValue);

            cellIndex = tblView->model()->index(selRow, 2);
            cellValue = tblView->model()->data(cellIndex, Qt::DisplayRole).toString();
            txtGen->setText(cellValue);

            cellIndex = tblView->model()->index(selRow, 3);
            cellValue = tblView->model()->data(cellIndex, Qt::DisplayRole).toString();
            txtYear->setText(cellValue);
        ...
        QObject::connect(btnCosCrudGui, &QPushButton::clicked, [&]() {
            addObserver(new CartCrudGui{ serv, observers });
            });

        QObject::connect(btnCosReadOnlyGui, &QPushButton::clicked, [&]() {
            addObserver(new CartReadOnlyGui{ serv, observers });
            });
        ...
        QObject::connect(btnGenerateCart, &QPushButton::clicked, [&]() {
            ...
            if (number > serv.getAll().size()) {
                ...
                serv.random(number);
                QMessageBox* box = new QMessageBox;
                box->setText("Ati introdus cartile in cos!" );
                box->show();
            }
            notifyObservers();
            });
